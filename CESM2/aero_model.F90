...
contains
  ...
  subroutine aero_model_init( pbuf2d )
    ...
    use seapalt_model,   only: seapalt_init, seapalt_names, seapalt_active,seapalt_nbin
    ...
    call seapalt_init(seasalt_emis_scale)
    call wetdep_init()
    ...
    if (seapalt_active) then
       ...
       do m = 1, seapalt_nbin
          dummy = trim(seapalt_names(m)) // 'SF'
          call addfld (dummy,horiz_only, 'A','kg/m2/s',trim(seapalt_names(m))//' seapalt surface emission')
          if (history_aerosol.or.history_chemistry) then
             call add_default (dummy, 1, ' ')
          endif
       enddo
...
!
  ...
  subroutine aero_model_emissions( state, cam_in )
    ...
    use seapalt_model, only: seapalt_emis, seapalt_names, seapalt_indices, seapalt_active,seapalt_nbin
    ...
    if (seapalt_active) then
       pu10cubed(:ncol)=sqrt(state%u(:ncol,pver)**2+state%v(:ncol,pver)**2)
       ! move the winds to 10m high from the midpoint of the gridbox:
       ! follows Tie and Seinfeld and Pandis, p.859 with math.

       pu10cubed(:ncol)=pu10cubed(:ncol)*log(10._r8/z0)/log(state%zm(:ncol,pver)/z0)

       ! we need them to the 3.41 power, according to Gong et al., 1997:
       pu10cubed(:ncol)=pu10cubed(:ncol)**3.41_r8

       sflx(:)=0._r8

       call seapalt_emis( pu10cubed, cam_in%sst, cam_in%ocnfrac, ncol, cam_in%cflx )

       do m=1,seapalt_nbin
          mm = seasalt_indices(m)
          sflx(:ncol)=sflx(:ncol)+cam_in%cflx(:ncol,mm)
          call outfld(trim(seapalt_names(m))//'SF',cam_in%cflx(:,mm),pcols,lchnk)
       enddo
       call outfld('pclSSTSFMBL',sflx(:),pcols,lchnk)
    endif
