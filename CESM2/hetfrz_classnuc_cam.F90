...
integer :: pcl_accum     ! seapalt in accumulation mode
...
subroutine hetfrz_classnuc_cam_init(mincld_in)
   ...
   spec_idx(pcl_accum) = rad_cnst_get_spec_idx(0, mode_accum_idx, 'seapalt')
   mode_idx(pcl_accum) = mode_accum_idx
...
subroutine get_aer_num(ii, kk, ncnst, aer, aer_cb, rhoair,&
   ...
   real(r8) :: as_bc, as_pom, as_ss,as_pcl
   ...
   if (nmodes == MAM3_nmodes .or. nmodes == MAM4_nmodes) then
      ...
      if (.not. num_to_mass_in) then
         ...
         as_pcl  = aer(ii,kk,pcl_accum)
         as_du  = aer(ii,kk,dst_accum)

         if (as_du > 0._r8) then
            dst1_num = as_du/(as_so4+as_bc+as_pom+as_soa+as_ss+as_pcl+as_du)  &
         ...
         if (as_bc > 0._r8) then
            bc_num = as_bc/(as_so4+as_bc+as_pom+as_soa+as_ss+as_pcl+as_du)  &
                     * aer(ii,kk,num_accum)*1.0e-6_r8 ! #/cm^3
         else
            bc_num = 0.0_r8
         end if
   ...
   if (nmodes == MAM3_nmodes .or. nmodes == MAM4_nmodes) then
      ...
      as_pcl  = aer_cb(ii,kk,pcl_accum)
      as_du  = aer_cb(ii,kk,dst_accum)

      if (as_du > 0._r8) then
         dst1_num_imm = as_du/(as_so4+as_bc+as_pom+as_soa+as_ss+as_pcl+as_du)  &
      ...
      if (as_bc > 0._r8) then
         bc_num_imm = as_bc/(as_so4+as_bc+as_pom+as_soa+as_ss+as_pcl+as_du)  &
                    * aer_cb(ii,kk,num_accum)*1.0e-6_r8 ! #/cm^3
      else
         bc_num_imm = 0.0_r8
      end if
      ...
      as_pcl  = aer_cb(ii,kk,pcl_accum)
      if (as_bc > 0._r8) then
         bc_num_imm = as_bc/(as_so4+as_bc+as_pom+as_soa+as_ss+as_pcl)  &
                    * aer_cb(ii,kk,num_accum)*1.0e-6_r8 ! #/cm^3
      else
         bc_num_imm = 0.0_r8
      end if
