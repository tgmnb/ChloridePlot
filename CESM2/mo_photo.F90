module mo_photo
  ...
  integer ::  psa1_ndx, psa2_ndx, psa3_ndx, psa4_ndx
...
contains
  ...
  subroutine photo_inti( xs_coef_file, xs_short_file, xs_long_file, rsf_file, &
    ...
    use seapalt_model, only : psslt_names=>seapalt_names, psslt_ncnst=>seapalt_nbin
    ...
    if (psslt_ncnst == 4) then
       psa1_ndx = get_spc_ndx( psslt_names(1) )
       psa2_ndx = get_spc_ndx( psslt_names(2) )
       psa3_ndx = get_spc_ndx( psslt_names(3) )
       psa4_ndx = get_spc_ndx( psslt_names(4) )
    endif
...
secant_in_bounds : &
          if( secant <= secant_limit ) then
             ...
             aersal(1,:) = aersal(2,:)
             if( psa1_ndx > 0 ) then
                aerpsal(pverp:2:-1,1) = vmr(i,:,psa1_ndx)
             else
                aerpsal(pverp:2:-1,1) = 0._r8
             end if
             if( psa2_ndx > 0 ) then
                aerpsal(pverp:2:-1,2) = vmr(i,:,psa2_ndx)
             else
                aerpsal(pverp:2:-1,2) = 0._r8
             end if
             if( psa3_ndx > 0 ) then
                aerpsal(pverp:2:-1,3) = vmr(i,:,psa3_ndx)
             else
                aerpsal(pverp:2:-1,3) = 0._r8
             end if
             if( psa4_ndx > 0 ) then
                aerpsal(pverp:2:-1,4) = vmr(i,:,psa4_ndx)
             else
                aerpsal(pverp:2:-1,4) = 0._r8
             end if
             aerpsal(1,:) = aerpsal(2,:)
             ...
             call photoin( ncdate, alat, along, &
                           ...
                           aersoa, aerant, aerso4, aersal, aerpsal, aerds, o3line, rh, &
