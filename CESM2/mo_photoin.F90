...
contains
  ...
  subroutine photoin( idate, alat, along, &
       ...
       asoa, aant, aso4, asal, apsal, ads, o3, rh, &
    ...
    real(r8), intent(in)    ::  apsal(pverp,4)
    ...
    real(r8)    :: dtpsal(pver,nw,4)
    real(r8)    :: ompsal(pver,nw,4)
    real(r8)    :: gpsal(pver,nw,4)
    ...
    dt_xdiag(6) = sum( dtpsal(:,16,1) + dtpsal(:,16,2) + dtpsal(:,16,3) + dtpsal(:,16,4) )
    dt_xdiag(7) = sum( dtds1(:,16) + dtds2(:,16) + dtds3(:,16) + dtds4(:,16) )
    dt_xdiag(8) = sum( dtsoa(:,16) )
    dt_xdiag(9) = sum( dt_xdiag(1:6) )
