...
contains
  ...
  subroutine rtlink( z, nw, albedo, zen, dsdh, &
    ...
    real(r8), intent(in)  :: dtpsal(pver,nw,4)
    real(r8), intent(in)  :: omsal(pver,nw,4)
    real(r8), intent(in)  :: ompsal(pver,nw,4)
    real(r8), intent(in)  :: gsal(pver,nw,4)
    real(r8), intent(in)  :: gpsal(pver,nw,4)
    ...
    real(r8)    :: dspsal1, dspsal2, dspsal3, dspsal4
    real(r8)    :: dapsal1, dapsal2, dapsal3, dapsal4
    real(r8)    :: dsds1, dads1
    real(r8)    :: dsds2, dads2
    real(r8)    :: dsds3, dads3
    real(r8)    :: dsds4, dads4
    ...
    wave_loop : do wn = 1,nw
       level_loop : do k = 1,pver
          ...
          ! sea palt
          !-----------------------------------------------------------------------
          wrk     = max( min( ompsal(k,wn,1),1._r8 ),smallest )
          dspsal1 = dtpsal(k,wn,1)*wrk
          dapsal1 = dtpsal(k,wn,1)*(1._r8 - wrk)

          wrk     = max( min( ompsal(k,wn,2),1._r8 ),smallest )
          dspsal2 = dtpsal(k,wn,2)*wrk
          dapsal2 = dtpsal(k,wn,2)*(1._r8 - wrk)

          wrk     = max( min( ompsal(k,wn,3),1._r8 ),smallest )
          dspsal3 = dtpsal(k,wn,3)*wrk
          dapsal3 = dtpsal(k,wn,3)*(1._r8 - wrk)

          wrk     = max( min( ompsal(k,wn,4),1._r8 ),smallest )
          dspsal4 = dtpsal(k,wn,4)*wrk
          dapsal4 = dtpsal(k,wn,4)*(1._r8 - wrk)
          !-----------------------------------------------------------------------
          ! summation
          !-----------------------------------------------------------------------
          dtsct = dtsct + dssal1 + dssal2 + dssal3 + dssal4 + dspsal1 + dspsal2 + dspsal3 + dspsal4
          dtabs = dtabs + dasal1 + dasal2 + dasal3 + dasal4 + dapsal1 + dapsal2 + dapsal3 + dapsal4
          ...
          if( dtsct /= smallest ) then
             ...
             g(kk,wn) = gcld(k,wn)*dscld     &
                      ...
                      + gsal(k,wn,3)*dssal3  &
                      + gsal(k,wn,4)*dssal4  &            
                      + gpsal(k,wn,1)*dspsal1  &
                      + gpsal(k,wn,2)*dspsal2  &
                      + gpsal(k,wn,3)*dspsal3  &
                      + gpsal(k,wn,4)*dspsal4
