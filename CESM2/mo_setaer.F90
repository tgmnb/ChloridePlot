...
!-----------------------------------------------------------------------------
      ...
      use seapalt_model,    only : psslt_names=>seapalt_names
...
!-----------------------------------------------------------------------------
      ...
      real(r8), intent(out)    :: dtpsal(pver,nw,4)
      real(r8), intent(out)    :: ompsal(pver,nw,4)
      real(r8), intent(out)    :: gpsal(pver,nw,4)
...
!  	... sea palts
!-----------------------------------------------------------------------------
      ndx = get_spc_ndx(psslt_names(1))
   
      if( ndx > 0 ) then
         mw = adv_mass( ndx )
         gpm2(:) = dz(:) * apsal(:pver,1) * mw 
         do wn = 1,nw
           do k = 1,pver
              rh_l  = rh_ndx(k)
              delrh = del_rh(k)
              dtpsal(k,wn,1) = gpm2(k)*(qext_sal01(rh_l,wn) + delrh*qext_dsal01(rh_l,wn))
              ompsal(k,wn,1) = ssa_sal01(rh_l,wn) + delrh*ssa_dsal01(rh_l,wn)
              gpsal(k,wn,1)  = asm_sal01(rh_l,wn) + delrh*asm_dsal01(rh_l,wn)
            end do
         end do
      else
        do wn = 1,nw
      ...
      ndx = get_spc_ndx(psslt_names(2))
      if( ndx > 0 ) then
         mw = adv_mass( ndx )
         gpm2(:) = dz(:) * apsal(:pver,2) * mw 
         do wn = 1,nw
           do k = 1,pver
              rh_l  = rh_ndx(k)
              delrh = del_rh(k)
              dtpsal(k,wn,2) = gpm2(k)*(qext_sal02(rh_l,wn) + delrh*qext_dsal02(rh_l,wn))
              ompsal(k,wn,2) = ssa_sal02(rh_l,wn) + delrh*ssa_dsal02(rh_l,wn)
              gpsal(k,wn,2)  = asm_sal02(rh_l,wn) + delrh*asm_dsal02(rh_l,wn)
            end do
         end do
      else
        do wn = 1,nw
      ...
      ndx = get_spc_ndx(psslt_names(3))
      if( ndx > 0 ) then
         mw = adv_mass( ndx )
         gpm2(:) = dz(:) * apsal(:pver,3) * mw 
         do wn = 1,nw
           do k = 1,pver
              rh_l  = rh_ndx(k)
              delrh = del_rh(k)
              dtpsal(k,wn,3) = gpm2(k)*(qext_sal03(rh_l,wn) + delrh*qext_dsal03(rh_l,wn))
              ompsal(k,wn,3) = ssa_sal03(rh_l,wn) + delrh*ssa_dsal03(rh_l,wn)
              gpsal(k,wn,3)  = asm_sal03(rh_l,wn) + delrh*asm_dsal03(rh_l,wn)
           end do
        end do
      else
        do wn = 1,nw
     ...
     ndx = get_spc_ndx(psslt_names(4))
     if( ndx > 0 ) then
         mw = adv_mass( ndx )
         gpm2(:) = dz(:) * apsal(:pver,4) * mw 
         do wn = 1,nw
           do k = 1,pver
              rh_l  = rh_ndx(k)
              delrh = del_rh(k)
              dtpsal(k,wn,4) = gpm2(k)*(qext_sal04(rh_l,wn) + delrh*qext_dsal04(rh_l,wn))
              ompsal(k,wn,4) = ssa_sal04(rh_l,wn) + delrh*ssa_dsal04(rh_l,wn)
              gpsal(k,wn,4)  = asm_sal04(rh_l,wn) + delrh*asm_dsal04(rh_l,wn)
           end do
        end do
      else
        do wn = 1,nw
