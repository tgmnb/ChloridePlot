...
!--------------------------------------------------------------
      ...
      integer, public, protected :: nSeaSalt=0, nDust=0,nSeaPalt=0
      ...
      integer, public, protected, allocatable ::  &
           ...
           lptr_pacl_a_amode(:),    lptr_pacl_cw_amode(:),&
...
!--------------------------------------------------------------
  subroutine modal_aero_data_reg
    ...
    allocate( &
         ...
         lptr_pacl_a_amode(ntot_amode), lptr_pacl_cw_amode(ntot_amode), &
    ...
    do m = 1, ntot_amode
       do l = 1, nspec_amode(m)
          ...
          if (xname_massptr(l,m)(:3) == 'pcl') nSeaPalt=nSeaPalt+1
...
!
     ...
     subroutine initaermodes_setspecptrs
       ...
       do m = 1, ntot_amode
          ...
          lptr_pacl_a_amode(m)   = init_val
          lptr_pacl_cw_amode(m)  = init_val
          ...
          do l = 1, nspec_amode(m)
             ...
             case('pcl')
                lptr_pacl_a_amode(m)  = lmassa
                lptr_pacl_cw_amode(m) = lmassc
             case('pom')
                pom_ndx = pom_ndx+1
       ...
       ! TESM: what is this?
       write(iulog,9000) 'seapalt   '
       do m = 1, ntot_amode
          call initaermodes_setspecptrs_write2( m,                    &
               lptr_pacl_a_amode(m), lptr_pacl_cw_amode(m),  'pcl' )
       end do
