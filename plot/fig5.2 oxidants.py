...
for box in cfg.box_list:
    ...
    boxdata.append((cfg.box_list[i][0], boxcolfindf, boxcolnochgdf))
    i += 1
...
for column in ['大气氧化物']:
    ...
    axes[0, 0].plot(colfindf['time'], colfindf[column], label='全球', linestyle='-', color='blue')
    axes[0, 0].plot(cncolfindf['time'], cncolfindf[column], label='中国', linestyle='-', color='orange')
    axes[0, 0].set_title(f'S1 {column} 的时间序列')
    axes[1, 0].set_title(f'S1 {column} 的时间序列')
    ...
    axes[1, 0].legend()
    # 绘制 SSP370 情景
    axes[0, 1].plot(colnochgdf['time'], colnochgdf[column], label='全球', linestyle='-', color='blue')
    axes[0, 1].plot(cncolnochgdf['time'], cncolnochgdf[column], label='中国', linestyle='-', color='orange')
    axes[0, 1].set_title(f'SSP370 {column} 的时间序列')
    axes[1, 1].set_title(f'SSP370 {column} 的时间序列')
    ...
    axes[0, 2].plot(colfindf['time'], (colfindf[column] - colnochgdf[column])/colnochgdf[column], label='全球', linestyle='-', color='blue')
    axes[0, 2].plot(cncolfindf['time'], (cncolfindf[column] - cncolnochgdf[column])/cncolnochgdf[column], label='中国', linestyle='-', color='orange')

    axes[0, 2].set_title(f'S1和SSP370下{column}的相对差异')
    axes[1, 2].set_title(f'S1和SSP370下{column}的相对差异')
    axes[0, 2].set_xlabel('Time')
    axes[1, 2].set_xlabel('Time')
    # axes[0, 2].set_ylabel(f'Relative Difference in {column} ($kg/m^{{2}}$)')
    axes[0, 2].set_ylabel(f'{column} 相对差异')
    axes[1, 2].set_ylabel(f'{column} 相对差异')
    axes[0, 2].legend()
    axes[1, 2].legend()
    # 添加标注
    ...
    #     plt.plot(colfindf['time'], colfindf[column], label='全球', linestyle='-', color='blue')
    ...
    # 保存图片
    plt.savefig(output_dir + f"5.2{column}.png")
